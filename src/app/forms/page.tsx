...
export default function FormsPage() {
  ...
  const [formPagination, setFormPagination] = useState<Record<string, { page: number; itemsPerPage: number }>>({})
  ...
  const updateForm = async () => {
    if (!editingForm || !newForm.title || newForm.fields.length === 0) return

    console.log('Updating form with ID:', editingForm.id)
    console.log('Update data:', newForm)
  ...
  const getFormPagination = (formId: string) => {
    return formPagination[formId] || { page: 1, itemsPerPage: 5 }
  }

  const updateFormPagination = (formId: string, page: number, itemsPerPage: number = 5) => {
    setFormPagination(prev => ({
      ...prev,
      [formId]: { page, itemsPerPage }
    }))
  }

  const getPaginatedSubmissions = (submissions: FormSubmission[], formId: string) => {
    const { page, itemsPerPage } = getFormPagination(formId)
                ...
                <div>
                  ...
                  {newForm.fields.length > 0 && (
                    <div className="space-y-2">
                      <h4 className="font-medium text-gray-900">Form Fields ({newForm.fields.length}):</h4>
                    ...
                    {form.submissionCount === 0 ? (
                      ...
                      expandedForms[form.id] && (
                        <div className="space-y-4">
                          {(() => {
                            const paginatedData = getPaginatedSubmissions(form.submissions || [], form.id)
                            return (
                              <>
                                {/* Table View */}
                                    ...
                                    <TableHeader>
                                      <TableRow>
                                        ...
                                        {form.fields.length > 3 && (
                                          <TableHead className="w-[100px]">More Fields</TableHead>
                                        )}
                                      </TableRow>
                                    </TableHeader>
                                    <TableBody>
                                          ...
                                          {form.fields.length > 3 && (
                                            <TableCell>
                                              <Button
                                                ...
                                                variant="outline"
                                                onClick={() => {
                                                  // Create a modal or expand view for all fields
                                                  alert(`View all fields for submission by ${submission.submitterName || 'Anonymous'}`)
                                                }}
                                              >
                                  ...
                                  <div className="flex items-center justify-between px-2">
                                    <div className="flex items-center space-x-2">
                                      <p className="text-sm text-gray-600">
                                        Showing {((paginatedData.currentPage - 1) * getFormPagination(form.id).itemsPerPage) + 1} to {Math.min(paginatedData.currentPage * getFormPagination(form.id).itemsPerPage, paginatedData.totalItems)} of {paginatedData.totalItems} entries
                                      </p>
                                    </div>
                                    <div className="flex items-center space-x-2">
                                      <Button
                                        size="sm"
                                        variant="outline"
                                        onClick={() => updateFormPagination(form.id, paginatedData.currentPage - 1)}
                                      ...
                                      <div className="flex items-center space-x-1">
                                        {Array.from({ length: paginatedData.totalPages }, (_, i) => i + 1)
                                          .filter(pageNum => {
                                            ...
                                            return pageNum === 1 || pageNum === paginatedData.totalPages || 
                                          ...
                                          .map((pageNum, index, array) => (
                                            <div key={pageNum} className="flex items-center">
                                              ...
                                              <Button
                                                size="sm"
                                                variant={pageNum === paginatedData.currentPage ? "primary" : "outline"}
                                                onClick={() => updateFormPagination(form.id, pageNum)}
